    ...
    public class EmployeesController : BaseController<Employee, EmployeeRepository, string>
    {
        ...
        public ActionResult Sign(LoginVM loginVM)
        {
            ...
            else
            {
                ...
                return Ok(new JWTokenVM { Token = idToken, Messages = "Selamat berhasil login" });
        ...
        public ActionResult GetGender()
        {
            ...
            if (getGender != null)
            {
                return Ok(new { status = HttpStatusCode.OK, result = getGender, messsage = "Data ditampilkan" });
            }
            else
            {
        ...
        public ActionResult GetRolee()
        {
            var getRolee = employeeRepository.GetRolee();

            if (getRolee != null)
            {
                return Ok(new { status = HttpStatusCode.OK, result = getRolee, messsage = "Data ditampilkan" });
            }
            else
            {
                return NotFound(new { status = HttpStatusCode.OK, result = getRolee, message = "Tidak ada data tampil" });
            }
        }
        ...
        public ActionResult GetSalary()
        {
            ...
            if (getSalary != null)
            {
                return Ok(new { status = HttpStatusCode.OK, result = getSalary, messsage = "Data ditampilkan" });
            }
            else
            {
                return NotFound(new { status = HttpStatusCode.OK, result = getSalary, message = "Tidak ada data tampil" });
            }
        }
    }
}
